...
export default class Options extends Component<Props, State> {
  constructor(props: Props){
    ...
    this.state = { depthOfChange: "question"};
  }
  ...
  saveChanges(handleFunction: (id: number) => void): void {
    if (this.props.hasFutureRepetitions) {
  ...
  render() {
    if (!this.props.editing && !this.props.deleting) {
    ...
    } else if (this.props.hasFutureRepetitions && this.state.depthOfChange === "question") {
      return (
        <ButtonsContainer>
          <button className="only-this" onClick={() => this.setState({depthOfChange: "onlyThis"})}>
            <FaRegSquare/> Only this one
          </button>
          <button className="this-and-future" onClick={() => this.setState({depthOfChange: "thisAndFuture"})}>
      ...
      return (
        <ButtonsContainer>
          ...
          <button className="cancel" onClick={() => { this.setState({depthOfChange: "question"}); this.resetItem(); }}>
            <FaTimes/> Cancel
          </button>
        </ButtonsContainer>
      );
...
interface State {
  depthOfChange: "question" | "onlyThis" | "thisAndFuture",
}
